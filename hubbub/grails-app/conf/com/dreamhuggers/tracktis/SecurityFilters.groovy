...
// Security filters restrict user access to private areas of the website
...
class SecurityFilters {
    ...
    def filters = {
        ...
        // their own notifications, not anybody else's.
        //
        // Because we're using a scaffolded controller for profiles,
        // we do user checks based on the domain instance ID, rather
        // than the username/userId
        profileChanges(controller: "notification", action: "(edit|update)") {
            before = {
                def currUserId = authenticateService.userDomain().id
	            def tempNotification = Notification.getById(params.id)
                if (tempNotification?.user?.id != currUserId) {
                    redirect(controller: "login", action: "denied")
                    return false
                }
                return true
