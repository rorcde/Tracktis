class DateTagLib {
    ...
    public static String getNiceDate(Date date) {
	    ...
	    public static String getRemainingTime(Date endDate) {

        def now = new Date()

        def diff = Math.abs(endDate.getTime() - now.getTime())
        ...
        if (niceTime.length() == 0) {
            ...
            niceTime += (endDate.getTime() > now.getTime()) ? "from now" : "ago"
        }

        return niceTime

    }
	def remainingTimeFromNow = {attrs ->

        def date = attrs.date

        out << getRemainingTime(date)
